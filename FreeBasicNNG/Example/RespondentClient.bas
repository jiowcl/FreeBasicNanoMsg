...
If (hLibrary > 0) And (NngInit(hLibrary) = NNG_OK) Then
    Dim Socket As Long
    Dim Rc As Long = NngRespondent0Open(hLibrary, @Socket)
    ...
    While 1
        ...
        If Rc = NNG_OK Then
            Dim lpszSurvey As String = BytesToString(lpszRecvBufferPtr, Cast(Long, sz))
            Dim lpszReply As String = "Answer to " & lpszSurvey

            Print("Survey: " & lpszSurvey)

            NngSend(hLibrary, Socket, StrPtr(lpszReply), Len(lpszReply), 0)
        End If
